...
def find_config_file() -> Optional[Path]:
    ...
    # Priority 2: mcp-config.json in skill's references folder
...
def load_config() -> dict:
    ...
    if not config_path:
        raise FileNotFoundError(
            "No MCP config found. Create mcp-config.json in "
            ".claude/skills/mcp-client/references/ or set MCP_CONFIG_PATH"
        )
...
Commands:
  ...
  call <server> <tool> '<json_args>'   Execute a tool with arguments

Examples:
  python mcp_client.py servers
  python mcp_client.py tools playwright
...
Config sources (checked in order):
  ...
  3. .claude/skills/mcp-client/references/mcp-config.json
  4. .mcp.json in current directory
  5. ~/.claude.json"""
    print(usage)
...
async def main():
    ...
    try:
        if command == "servers":
            ...
            if len(sys.argv) < 3:
                print_error("Usage: tools <server>", "usage")
                sys.exit(1)
